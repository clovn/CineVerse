    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                ...
                if state.isLoading {
                    ...
                    if listToDisplay.isEmpty {
                        Spacer()
                        Text(state.selectedTab == .favorites ? "No favorite movies added." : "Watch Later list is empty.")
                        ...
                        List {
                            ...
                            .onDelete { indexSet in
                                for index in indexSet {
                                    ...
                                    viewModel.sendIntent(intent: WatchlistIntent.RemoveMovie(movieId: movie.id, tab: state.selectedTab))
                                }
                            }
                        }
            ...
            .alert(isPresented: $showToast) {
                Alert(title: Text("Info"), message: Text(toastMessage ?? ""), dismissButton: .default(Text("OK")))
            }
        }
        .task {
